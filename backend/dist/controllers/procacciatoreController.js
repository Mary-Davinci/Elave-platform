...
exports.uploadProcacciatoriFromExcel = exports.deleteProcacciatore = exports.updateProcacciatore = exports.createProcacciatore = exports.getProcacciatoreById = exports.getProcacciatori = void 0;
const Procacciatore_1 = __importDefault(require("../models/Procacciatore"));
...
const getProcacciatoreById = async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const procacciatore = await Procacciatore_1.default.findById(id);
        if (!procacciatore) {
        ...
        upload(req, res, async (err) => {
            ...
            try {
                ...
                if (lastName !== undefined)
                    procacciatore.lastName = lastName;
                if (email !== undefined)
                    procacciatore.email = email;
                if (phone !== undefined)
                    procacciatore.phone = phone;
                if (address !== undefined)
                    procacciatore.address = address;
                if (city !== undefined)
                    procacciatore.city = city;
                if (postalCode !== undefined)
                    procacciatore.postalCode = postalCode;
                if (province !== undefined)
                    procacciatore.province = province;
                if (taxCode !== undefined)
                    procacciatore.taxCode = taxCode;
                if (agreementPercentage !== undefined)
                    procacciatore.agreementPercentage = parseFloat(agreementPercentage);
                if (specialization !== undefined)
                    procacciatore.specialization = specialization;
                if (notes !== undefined)
...
exports.updateProcacciatore = updateProcacciatore;
// Delete a procacciatore
const deleteProcacciatore = async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: "User not authenticated" });
        }
        ...
        if (procacciatore.contractFile?.path && fs_1.default.existsSync(procacciatore.contractFile.path)) {
            fs_1.default.unlinkSync(procacciatore.contractFile.path);
        }
        if (procacciatore.idDocumentFile?.path && fs_1.default.existsSync(procacciatore.idDocumentFile.path)) {
            fs_1.default.unlinkSync(procacciatore.idDocumentFile.path);
        }
        await procacciatore.deleteOne();
        // Update dashboard stats
        const DashboardStats = require("../models/Dashboard").default;
        await DashboardStats.findOneAndUpdate({ user: req.user._id }, { $inc: { procacciatori: -1 } }, { new: true });
...
exports.deleteProcacciatore = deleteProcacciatore;
// Upload procacciatori from Excel file
const uploadProcacciatoriFromExcel = async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: "User not authenticated" });
        }
...
exports.uploadProcacciatoriFromExcel = uploadProcacciatoriFromExcel;
//# sourceMappingURL=procacciatoreController.js.map