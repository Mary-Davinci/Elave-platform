...
const storage = multer.diskStorage({
  destination: (req: Request, file: Express.Multer.File, cb: (error: Error | null, destination: string) => void) => {
    const uploadDir = path.join(__dirname, '../uploads/segnalatori');
...
export const getSegnalatori: CustomRequestHandler = async (req, res) => {
  try {
    if (!req.user) {
    ...
    const segnalatori = await Segnalatore.find(query).sort({ createdAt: -1 });
...
export const getSegnalatoreById: CustomRequestHandler = async (req, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: "User not authenticated" });
    }
    ...
    const segnalatore = await Segnalatore.findById(id);
...
export const createSegnalatore: CustomRequestHandler = async (req, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: "User not authenticated" });
    }
    ...
    upload(req, res, async (err) => {
      ...
      try {
        ...
        const newSegnalatore = new Segnalatore({
          firstName,
          lastName,
          email,
          phone: phone || '',
...
export const updateSegnalatore: CustomRequestHandler = async (req, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: "User not authenticated" });
    }
    ...
    upload(req, res, async (err) => {
      ...
      try {
        ...
        if (firstName !== undefined) segnalatore.firstName = firstName;
        if (lastName !== undefined) segnalatore.lastName = lastName;
        if (email !== undefined) segnalatore.email = email;
        if (phone !== undefined) segnalatore.phone = phone;
        if (address !== undefined) segnalatore.address = address;
        if (city !== undefined) segnalatore.city = city;
        if (postalCode !== undefined) segnalatore.postalCode = postalCode;
        if (province !== undefined) segnalatore.province = province;
        if (taxCode !== undefined) segnalatore.taxCode = taxCode;
        if (agreementPercentage !== undefined) segnalatore.agreementPercentage = parseFloat(agreementPercentage);
        if (specialization !== undefined) segnalatore.specialization = specialization;
        if (notes !== undefined) segnalatore.notes = notes;
        if (isActive !== undefined) segnalatore.isActive = Boolean(isActive);
...
export const deleteSegnalatore: CustomRequestHandler = async (req, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: "User not authenticated" });
    }
    ...
    if (segnalatore.contractFile?.path && fs.existsSync(segnalatore.contractFile.path)) {
      fs.unlinkSync(segnalatore.contractFile.path);
    }
    if (segnalatore.idDocumentFile?.path && fs.existsSync(segnalatore.idDocumentFile.path)) {
      fs.unlinkSync(segnalatore.idDocumentFile.path);
    }
...
export const uploadSegnalatoriFromExcel: CustomRequestHandler = async (req, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: "User not authenticated" });
    }
      ...
      try {
        ...
        for (const [index, row] of (data as any[]).entries()) {
          try {
            ...
            const segnalatoreRecord = new Segnalatore(segnalatoreData);
        ...
        if (segnalatori.length > 0) {
          ...
          await DashboardStats.findOneAndUpdate(
            { user: req.user._id },
            { $inc: { segnalatori: segnalatori.length } },
            { new: true, upsert: true }
          );
