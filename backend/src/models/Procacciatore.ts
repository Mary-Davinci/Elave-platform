...
const ProcacciatoreSchema = new Schema({
  firstName: { 
    type: String, 
...
ProcacciatoreSchema.statics.findByUser = function(userId: string) {
  return this.find({ user: userId }).sort({ createdAt: -1 });
};

ProcacciatoreSchema.statics.findByTaxCode = function(taxCode: string, userId?: string) {
...
const Procacciatore = mongoose.model<IProcacciatoreDocument>('Procacciatore', ProcacciatoreSchema);
