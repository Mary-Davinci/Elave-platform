...
import { getSupplierById, updateSupplier, SupplierFormData } from '../services/supplierService';
...
const EditSupplier: React.FC = () => {
  ...
  // Check if user is authenticated and load supplier data
  useEffect(() => {
    if (!isAuthenticated) {
      navigate('/login');
      return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Si è verificato un errore durante l\'aggiornamento del fornitore');
    } finally {
      setLoading(false);
    }
  };
