...
import { ProcacciatoreFormData, FormTemplate } from '../types/interfaces';
import { procacciatoreService } from '../services/procacciatoreService';
import { useAuth } from '../contexts/AuthContext'; // Import AuthContext

const ProcacciatoreForm: React.FC = () => {
  const { user } = useAuth(); // Get user from AuthContext
  
  const [formData, setFormData] = useState<ProcacciatoreFormData>({
    firstName: '',
    lastName: '',
    email: '',
    phone: '',
  ...
  useEffect(() => {
    const fetchFormTemplates = async () => {
      try {
        ...
        // Fetch templates specifically for Procacciatore
        const response = await fetch(`${apiBaseUrl}/api/form-templates/procacciatore`, {
          headers: {
        ...
        if (response.ok) {
          ...
          console.log('Fetched Procacciatore templates:', templates);
  ...
  const handleUploadTemplate = async (type: 'contract' | 'id') => {
    ...
    try {
      ...
      uploadFormData.append('category', 'procacciatore'); // Add category for Procacciatore
      ...
      if (response.ok) {
        ...
        const templatesResponse = await fetch(`${apiBaseUrl}/api/form-templates/procacciatore`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
  ...
  const handleDownloadTemplate = async (type: 'contract' | 'id') => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/api/form-templates/download/procacciatore/${type}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
      ...
      if (response.ok) {
        ...
        const filename = template?.originalName || `procacciatore_${type}_template.pdf`;
        a.download = filename;
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, fileType: 'contract' | 'id') => {
    ...
    if (file) {
      ...
      const allowedTypes = procacciatoreService.getDocumentAllowedTypes();
      const maxSize = 5 * 1024 * 1024; // 5MB
      
      const validation = procacciatoreService.validateFile(file, allowedTypes, maxSize);
  ...
  const validateForm = (): boolean => {
    ...
    if (!procacciatoreService.validatePercentage(formData.agreementPercentage)) {
      newErrors.push("Percentuale accordo cannot exceed 100%");
    }
    
    if (formData.email && !procacciatoreService.validateEmail(formData.email)) {
      newErrors.push("Please enter a valid email address");
    }
    
    if (formData.taxCode && !procacciatoreService.validateTaxCode(formData.taxCode)) {
      newErrors.push("Please enter a valid Italian tax code");
    }
    
    if (formData.postalCode && !procacciatoreService.validatePostalCode(formData.postalCode)) {
  ...
  const handleSubmit = async (e: React.MouseEvent) => {
    ...
    try {
      // Use service to create procacciatore
      ...
      await procacciatoreService.createProcacciatore(procacciatoreData);
      
      setSuccessMessage('Procacciatore creato con successo!');
  ...
  return (
    <div className="new-company-container">
      <div className="new-company-header">
        <h1 className="page-title">Nuovo Procacciatore</h1>
      </div>
      ...
      {isAdmin ? (
        ...
        <div className="template-management-section" style={{ 
          ...
          <div style={{ 
            ...
            <h3 style={{ 
              ...
              Gestione Moduli Procacciatore (Admin)
            </h3>
          </div>
        ...
        <div className="template-download-section" style={{ 
          ...
          <p style={{ 
            ...
            Scarica i moduli necessari per completare la procedura di registrazione Procacciatore.
          </p>
          ...
          <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
            <button
              ...
              style={{
                backgroundColor: getAvailableTemplate('contract') ? '#66bb6a' : '#6c757d',
                color: 'white',
                padding: '14px 24px',
                border: 'none',
                borderRadius: '8px',
                ...
                fontWeight: '600',
                transition: 'all 0.3s ease',
                boxShadow: getAvailableTemplate('contract') ? '0 2px 4px rgba(102, 187, 106, 0.3)' : 'none',
                minWidth: '200px'
              }}
              onMouseOver={(e) => {
            ...
            <button
              ...
              style={{
                backgroundColor: getAvailableTemplate('id') ? '#42a5f5' : '#6c757d',
                color: 'white',
                padding: '14px 24px',
                border: 'none',
                borderRadius: '8px',
                ...
                fontWeight: '600',
                transition: 'all 0.3s ease',
                boxShadow: getAvailableTemplate('id') ? '0 2px 4px rgba(66, 165, 245, 0.3)' : 'none',
                minWidth: '200px'
              }}
              onMouseOver={(e) => {
      ...
      <div className="new-company-form">
        ...
        <div className="upload-form-container">
          ...
          <div className="form-group">
            ...
            {contractFile && (
              <div style={{ marginTop: '5px', fontSize: '12px', color: '#666' }}>
                Size: {procacciatoreService.getFileSizeString(contractFile.size)}
              </div>
            )}
            {contractFile && (
              <button
          ...
          <div className="form-group">
            ...
            {idDocumentFile && (
              <div style={{ marginTop: '5px', fontSize: '12px', color: '#666' }}>
                Size: {procacciatoreService.getFileSizeString(idDocumentFile.size)}
              </div>
            )}
            {idDocumentFile && (
              <button
        ...
        <div className="form-actions">
          ...
          >
            {isSubmitting ? 'Creazione in corso...' : 'Crea Procacciatore'}
          </button>
        </div>
      </div>
    </div>
...
export default ProcacciatoreForm;
